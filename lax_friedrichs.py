...
def initial_conditions(grid):  # calculate and impose initial conditions
    ...
    U = 100*np.exp(-((X + 1)**2 + Y**2)/0.01)
    return U


def boundary_conditions(U):  # calculate and the boundary conditions
    U[:, 0] = 0.0
    U[:, -1] = 0.0
    U[0, :] = 0.0
    U[-1, :] = 0.0
...
def my_LF(grid, U_p, p, dx, dy, dt):  # my implementation of Lax-Friedrichs scheme
    ...
    for i in range(1, len(X) - 1):
        for j in range(1, len(Y) - 1):
            U_n[i, j] = 1 / 4 * (U_p[i + 1, j] + U_p[i - 1, j] + U_p[i, j + 1] + U_p[i, j- 1]) - dt * ((U_p[i + 1, j] - U_p[i - 1, j]) / (2 * dx)- 2 * p * X[i, j] * (U_p[i, j + 1] - U_p[i, j - 1])/(2* dy))
    return U_n
...
def main():
    ...
    print(
        f"Total number of calculations: {x_int_len/step_x*y_int_len/step_y*num_t_steps}\n"
    )
    ...
    for i in range(num_t_steps + 1):
        ...
        U_next = my_LF(grid=my_grid, U_p=U_prev, p=p, dx=step_x, dy=step_y, dt=step_t)
        ...
        sys.stdout.write(
            f"\rProgress: {i+1}/{num_t_steps}    :    {int(((i+1)*100/num_t_steps)*100)/100}%     "
        )
        sys.stdout.flush()
    ...
    # report
    print(
        f"Elapsed time: {int(elapsed_time/3600)}:{int((elapsed_time-int(elapsed_time/3600)*3600)/60)}:{elapsed_time - int(elapsed_time/3600)*3600 - int(elapsed_time/60)*60}"
    )
